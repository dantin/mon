...
void show_status_of(const char *pidfile)
{
    ...
    if (alive(pid)) {
        ...
        printf("\e[90m%d\e[0m : \e[31malive\e[0m\n", pid);
    }
...
{
    int logfd = open(file, O_WRONLY | O_CREAT | O_APPEND, 0644);
...
{
    char buf[BUF_SIZE] = { 0 };
    snprintf(buf, BUF_SIZE, "%s %d", monitor->on_restart, pid);
...
{
    char buf[BUF_SIZE] = { 0 };
    snprintf(buf, BUF_SIZE, "%s %d", monitor->on_error, pid);
    log("on error `%s`", buf);
    int status = system(buf);
    if (status) log("exit(%d)", status);
}
 ...
 */
int64_t ms_since_last_restart(monitor_t *monitor)
{
 ...
 */
int attemps_exceeded(monitor_t *monitor, int64_t ms)
{
    monitor->attemps++;
    monitor->clock -= ms;
    ...
    // reset
    if (monitor->clock <= 0) {
        monitor->clock = 60 * 1000;
        monitor->attemps = 0;
        return 0;
    }

    // all good
    if (monitor->attemps < monitor->max_attempts) return 0;
...
error: {
           ...
           log("%d attemps remaining", monitor->max_attempts - monitor->attemps);

           if (attemps_exceeded(monitor, ms)) {
               char *time = milliseconds_to_long_string(60 * 1000 - monitor->clock);
               log("%d restart within %s, bailing", monitor->max_attempts, time);
 ...
 * --sleep <sec>
 */
static void on_sleep(command_t *self)
{
    monitor_t *monitor = (monitor_t *) self->data;
 ...
 * --daemonize
 */
static void on_daemonize(command_t *self)
{
    monitor_t *monitor = (monitor_t *) self->data;
    monitor->daemon = 1;
}
 ...
 * --pidfile <path>
 */
static void on_pidfile(command_t *self)
{
    monitor_t *monitor = (monitor_t *) self->data;
 ...
 * --mon-pidfile <path>
 */
static void on_mon_pidfile(command_t *self)
{
    monitor_t *monitor = (monitor_t *) self->data;
 ...
 * --status
 */
static void on_status(command_t *self)
{
    monitor_t *monitor = (monitor_t *) self->data;
 ...
 * --prefix
 */
static void on_prefix(command_t *self)
{
    prefix = self->arg;
}
 ...
 * --on-restart <cmd>
 */
static void on_restart(command_t *self)
{
    monitor_t *monitor = (monitor_t *) self->data;
 ...
 * --on-error <cmd>
 */
static void on_error(command_t *self)
{
    monitor_t *monitor = (monitor_t *) self->data;
 ...
 * --attempts <n>
 */
static void on_attempts(command_t *self)
{
    monitor_t *monitor = (monitor_t *) self->data;
